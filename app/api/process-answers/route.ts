...
`;
      ...
      const systemPrompt = "You are an expert JEE teacher. You must analyze the question mathematically and accurately. Respond with ONLY the final answer (A, B, C, D, or the closest integer). Do not include any explanation, units, or extra text.";

      try {
        const response = await callAI(prompt, systemPrompt, false, q.imageUrl);
        let cleanedAnswer = response.trim().replace(/^['"]|['"]$/g, '').toUpperCase();
        // Fallback for safety
        if (q.type === 'MCQ' && !['A', 'B', 'C', 'D'].includes(cleanedAnswer)) {
            cleanedAnswer = 'A'; // Hard fallback if AI hallucinates
        } else if (q.type === 'NUMERICAL') {
            // Strip any alphabets or equal signs just in case AI still says "Ratio = 1"
            cleanedAnswer = cleanedAnswer.replace(/[a-zA-Z\=\s]/g, '');
        }

        await prisma.question.update({
          where: { id: q.id },
          data: { correctAnswer: cleanedAnswer, explanation: "Generated by AI" }
        })
      } catch (err) {
